...
def intent_router_node(state: GraphState) -> Dict[str, Any]:
    # Route checks cached intent first
    ...
    prompt = (
        ...
        "Analyze the user's question and categorize it into one of three intents:\n\n"
        "- 'sql': If the query asks for metadata, counts of articles, statistics, database states, "
        ...
        f"User Query: \"{query}\"\n"
        "Return ONLY one word: either 'sql', 'rag', or 'hybrid'."
    )
...
def crag_grader_node(state: GraphState) -> Dict[str, Any]:
    ...
    # If relevance is low (< 0.7), activate Tavily/DuckDuckGo search fallback
    if relevance_score < 0.7:
        print(f"CRAG: Relevance score ({relevance_score}) is low. Running Web Search Fallback...")
        try:
            with DDGS() as ddgs:
                search_results = [r["body"] for r in ddgs.text(query, max_results=3)]
                if search_results:
...
def answer_generator_node(state: GraphState) -> Dict[str, Any]:
    ...
    context_str = ""
    if intent == "sql":
        context_str = json.dumps(state["sql_results"], indent=2)
    else:
    ...
    if intent == "sql":
        ...
        prompt = (
            ...
            "based ONLY on the spotlighted context documents. If the answer is not in the context, say "
...
def self_rag_reflector_node(state: GraphState) -> Dict[str, Any]:
    ...
    # SQL responses or security blocks don't need context validation
    if intent == "sql" or "I do not have enough daily news" in answer or "Security block" in answer:
